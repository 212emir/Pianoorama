class Piano{
  ...
  Piano(int qL, int cL, String mN, float rRS, float rW, float v, int o, boolean m, float mS){
  ...
  void keyPress(char keyy){
    ...
    if(octave > 7) {
      octave = 7; //allows the user to only increase the octave up to the seventh octave
    }
    if(octave < 0) {
    ...
    reverb.process(file);
    reverb.room(this.reverbRoomSize);
    reverb.wet(this.reverbWetness);
    playKey(keyy);
